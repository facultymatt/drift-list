...
---
USER
Below are exchanges from Claude Code sessions, in the order they occurred. Consecutive exchanges are usually from the same session, but the window may span several unrelated pieces of work.

Exchanges come in two kinds, and the speaker labels tell you which:

- **`Engineer:` → `Claude:`** — a turn the engineer typed, and Claude's reply.
- **`Claude (to subagent):` → `Subagent:`** — marked *(subagent task)*. Claude wrote that request to dispatch a subagent. **The engineer never wrote it and may never have read the reply.**

This distinction governs attribution:

- Never set `driver: "engineer"` or `"joint"` on the strength of a `Claude (to subagent):` turn, and never treat one as `engagement: "directed"` or `"questioned"`. The engineer did not say it.
- Never quote a `Claude (to subagent):` turn as a stated constraint. Constraints come from `Engineer:` turns only.
- Do still record the work itself. A task delegated to a subagent is work the engineer did not do by hand, which makes it exactly the kind of thing this log exists to surface. Record it with `driver: "claude"` and `engagement: "none"` unless a nearby `Engineer:` turn shows they directed or questioned it.
- Set `"surface": "subagent"` on entries that come from subagent exchanges, and `"surface": "direct"` otherwise.

If an engineer turn asked for something and a subagent then carried it out, that is one decision the engineer drove, not two. Attribute it to the engineer and set `"surface": "subagent"`.
...
{
  "concepts": ["taxonomy labels that appear anywhere in these sessions, including in code that raised no issue — this is a coverage list, not a problem list"],
