...
import type { Exchange } from './types5.js'
...
export interface Decision {
  id: string
  /**
   * Where the work happened. Optional: older artifacts lack it, and the model
   * sometimes sheds schema fields on large windows, so absence means 'direct'
 ...
 *
 * Subagent exchanges are labelled rather than hidden. The speaker labels carry
 * the distinction: an engineer turn is "Engineer:", a dispatch turn is
 * "Claude (to subagent):". Phase 1 needs that difference to avoid crediting the
 * engineer with a request Claude wrote — v4 solved the same problem by dropping
 * these exchanges, which also dropped the work itself from the record.
 */
function formatExchanges(exchanges: Exchange[]): string {
  const sections: string[] = []
  for (const ex of exchanges) {
    if (ex.source === 'subagent') {
      sections.push(`**[${ex.timestamp}]** *(subagent task)*`)
      sections.push(`Claude (to subagent): ${ex.userText}`)
      sections.push(`Subagent: ${ex.assistantText}`)
    } else {
      sections.push(`**[${ex.timestamp}]**`)
      sections.push(`Engineer: ${ex.userText}`)
