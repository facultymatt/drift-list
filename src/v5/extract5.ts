...
import type { SessionFile } from './discover5.js'
import type { Exchange, ExchangeSource, RawRecord, ContentBlock } from './types5.js'
...
export async function extractWindowExchanges(
  ...
  return {
    exchanges: all,
    engineerExchanges: all.filter(e => e.source === 'engineer').length,
    subagentExchanges: all.filter(e => e.source === 'subagent').length,
  }
}

/** What extraction found, split by who wrote the request half. */
export type ExtractResult = {
  exchanges: Exchange[]
  /** Exchanges whose request the engineer typed */
  engineerExchanges: number
  /** Exchanges whose request Claude wrote to dispatch a subagent */
...
async function parseSession(filePath: string): Promise<Exchange[]> {
  ...
  let pendingUser: { text: string; cwd: string; sessionId: string; timestamp: string; source: ExchangeSource } | null = null
  ...
  for (const record of records) {
    if (record.type === 'user') {
      ...
      pendingUser = {
        text,
        cwd,
        // A `type: "user"` record inside a sidechain is Claude's dispatch
        // prompt to a subagent, not something the engineer typed.
        source: record.isSidechain === true ? 'subagent' : 'engineer',
