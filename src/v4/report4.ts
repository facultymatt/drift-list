...
import type { DecisionLog, RenderOutput, Decision } from './pipeline4.js'

export type Report4Meta = {
  start: Date
  end: Date
  sessionCount: number
  cwds: string[]
 ...
 */
export async function writeReport4(opts: Report4Options): Promise<void> {
...
function renderMarkdown(log: DecisionLog, render: RenderOutput, meta: Report4Meta): string {
  const fmt = (d: Date) =>
  ...
  s.push('# Skill Drift — Session Analysis (v4)')
  s.push(
  ...
  for (const d of log.decisions) {
    s.push(`### ${d.id}`)
    s.push('')
    s.push(`${d.kind} · driver: ${d.driver} · engagement: ${d.engagement}`)
    s.push('')
    s.push(md(d.what))
    s.push('')
  ...
  s.push('---')
  s.push(`*Generated by skill-drift (v4) · ${new Date().toISOString()}*`)
