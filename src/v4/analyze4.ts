...
import { discoverSessions } from './discover4.js'
import { extractWindowExchanges } from './extract4.js'
import { runDecisionLog, runRender } from './pipeline4.js'
import { writeReport4 } from './report4.js'
import type { DecisionLog } from './pipeline4.js'
...
async function main3(): Promise<void> {
  ...
  const { exchanges, sidechainRecords, sidechainFiles } = await extractWindowExchanges(sessions, start, end, log)
  if (exchanges.length === 0) {
    log('No exchanges found in the given time window.')
    process.exit(1)
  }

  if (sidechainRecords > 0) {
    log(
      `Skipped ${sidechainRecords} subagent record(s) across ${sidechainFiles.length} file(s) ` +
      `— these are Claude prompting Claude, not the engineer.`
    )
  }
...
function log(msg: string): void {
  process.stderr.write(`[skill-drift-v4] ${msg}\n`)
}

main3().catch(err => {
  process.stderr.write(
    `[skill-drift-v4] Error: ${err instanceof Error ? err.message : String(err)}\n`
  )
  process.exit(1)
})
