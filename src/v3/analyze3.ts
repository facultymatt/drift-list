...
import { extractWindowExchanges } from '../v2/extract2.js'
import { runDecisionLog, runRender } from './pipeline3.js'
import { writeReport3 } from './report3.js'
import type { DecisionLog } from './pipeline3.js'
...
async function main3(): Promise<void> {
  ...
  // A project filter narrows what phase 1 ever sees. It's a deliberate focusing
  // tool, but it trades away cross-cutting coverage — design decisions whose
  // reasoning spans a whole session can start in turns that don't touch the
  // filtered project's files, and get dropped before they're recorded. Make the
  // tradeoff visible whenever the filter is on, so a thin report isn't mistaken
  // for a quiet session. All-projects is the truthful default.
  if (args.projects && args.projects.length > 0) {
    log(
      `Note: --project ${args.projects.join(', ')} is active. ` +
      `Only sessions touching these projects were analyzed; cross-project and ` +
      `whole-session design decisions may be under-reported. Omit --project for full coverage.`
    )
  }
  ...
  // Attach run provenance so the artifacts are self-describing: the window, the
  // exact command, and any project filter. This is added by the CLI, not the
  ...
  decisionLog.run = {
    ...
    projectFilter: args.projects && args.projects.length > 0 ? args.projects : undefined,
...
function log(msg: string): void {
  process.stderr.write(`[drift-list-v3] ${msg}\n`)
}

main3().catch(err => {
  process.stderr.write(
    `[drift-list-v3] Error: ${err instanceof Error ? err.message : String(err)}\n`
  )
  process.exit(1)
})
