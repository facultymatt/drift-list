...
import type { DecisionLog, RenderOutput, Decision } from './pipeline6.js'

export type Report6Meta = {
  start: Date
  end: Date
  sessionCount: number
  cwds: string[]
 ...
 */
export async function writeReport6(opts: Report6Options): Promise<void> {
...
function renderMarkdown(log: DecisionLog, render: RenderOutput, meta: Report6Meta): string {
  const fmt = (d: Date) =>
  ...
  s.push('# Skill Drift — Session Analysis (v6)')
  s.push(
  ...
  for (const item of render.watch_list) {
    ...
    if (d) {
      // Same metadata the full decision log carries. Surfaced here because the
      // watch list is what most readers actually read, and driver/engagement is
      // the whole point of the tool — a `claude · none` item is a rep not taken,
      // which reads very differently from one the engineer directed.
      const meta = [`kind: ${d.kind} · driver: ${d.driver} · engagement: ${d.engagement}`]
      if (d.concepts?.length) meta.push(`concepts: ${d.concepts.join(', ')}`)
      s.push(`*${md(meta.join(' · '))}*`)
    }
    s.push('')
  ...
  for (const d of log.decisions) {
    s.push(`### ${d.id}`)
    s.push('')
    s.push(`kind: ${d.kind} · driver: ${d.driver} · engagement: ${d.engagement}`)
    s.push('')
    s.push(md(d.what))
    s.push('')
  ...
  s.push('---')
  s.push(`*Generated by skill-drift (v6) · ${new Date().toISOString()}*`)
